    ...
    return (
        <div className="min-h-screen bg-gray-50 px-3 py-4">
            ...
            <div className="bg-white rounded-lg p-4 mb-4 shadow-sm">
                <h2 className="text-xl font-bold mb-1 relative inline-block text-gray-800">
                    <span
                        ref={underlineRef}
                ...
                <span className="text-xl font-bold text-gray-800">sheet</span>
            ...
            <div className="bg-white rounded-lg p-4 mb-4 shadow-sm space-y-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        className="w-full rounded-md p-3 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#018ABE] text-sm"
                    />
                </div>

                {/* Manager Selection */}
                <div className="relative">
                    <label className="block mb-2 font-medium text-gray-700 text-sm">Select Manager</label>
                    <button
                        onClick={() => !isFilledTimesheet && setShowDropdown(!showDropdown)}
                        className={`w-full border ${validationErrors.managers ? 'border-red-500' : 'border-gray-300'} rounded-md px-4 py-3 flex items-center justify-between text-sm ${isFilledTimesheet ? 'bg-gray-100 cursor-not-allowed' : 'cursor-pointer'}`}
                        disabled={isFilledTimesheet}
                    >
                        <span className="text-gray-800">{`All Selected (${selectedManagers.length})`}</span>
                        <FiChevronDown className="text-gray-600 text-lg" />
                    </button>
                    {validationErrors.managers && (
                        <p className="text-red-500 text-xs mt-1">{validationErrors.managers}</p>
                    )}
                    {showDropdown && !isFilledTimesheet && (
                        <div
                            className="absolute top-full mt-1 bg-white border border-gray-200 rounded-md w-full z-10 shadow-lg"
                            onMouseLeave={() => setShowDropdown(false)}
                        >
                            {["Awab Fakih", "Ayaan Raje", "Prashant Patil"].map((managerName) => (
                                <label key={managerName} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-100 cursor-pointer text-sm">
                                    <input
                ...
                {/* Project Name */}
                <div>
                    <label className="block mb-2 font-medium text-gray-700 text-sm">Project Name</label>
                    <input
                        type="text"
                        ...
                        className={`w-full border ${validationErrors.projectName ? 'border-red-500' : 'border-gray-300'} rounded-md px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#018ABE] text-sm ${isFilledTimesheet ? 'bg-gray-100' : ''}`}
                ...
                <button
                    ...
                    className={`w-full py-3 rounded-lg text-sm font-medium ${isFilledTimesheet ? 'bg-gray-400 text-gray-600' : 'bg-[#018ABE] text-white'}`}
                    disabled={isFilledTimesheet}
                >
                    Add Row
                </button>
                    ...
                    >
                        ...
                        <div className="flex justify-between items-center mb-3">
                            ...
                            <button
                                ...
                                className={`p-2 rounded-full ${isFilledTimesheet ? 'opacity-30 cursor-not-allowed' : 'hover:bg-red-50'}`}
                                disabled={isFilledTimesheet}
                            >
                        ...
                        <div className="mb-3">
                            ...
                            <select 
                                className={`w-full border border-gray-300 rounded-md p-2 text-sm ${isFilledTimesheet ? 'bg-gray-100 cursor-not-allowed' : 'cursor-pointer'}`}
                                value={item.bucket}
                        ...
                        <div className="mb-3">
                            ...
                            <textarea
                                className={`w-full border ${validationErrors[`task-${index}`] ? 'border-red-500' : 'border-gray-300'} rounded-md p-2 text-sm resize-none ${isFilledTimesheet ? 'bg-gray-100' : ''}`}
                                rows="3"
                                value={item.task}
            ...
            <div className="bg-white rounded-lg p-4 shadow-sm mb-4">
                <div className="flex justify-between items-center">
                    ...
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${isLessThanEightHours ? "bg-red-100 text-red-800" : "bg-green-100 text-green-800"}`}>
                        {totalTime}
                    </span>
                </div>
            </div>
            ...
            <div className="sticky bottom-4">
                <button
                    ...
                    className={`w-full py-4 rounded-lg font-semibold text-lg ${isFilledTimesheet ? 'bg-gray-400 cursor-not-allowed text-gray-600' : 'bg-[#018ABE] text-white shadow-lg'}`}
                    disabled={isFilledTimesheet}
                >
                    Submit Timesheet
                </button>

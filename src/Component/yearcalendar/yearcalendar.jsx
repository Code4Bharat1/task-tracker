...
const Yearcalendar = () => {
  ...
  const getDateStyling = (year, month, day) => {
    ...
    let classes =
      "rounded h-6 md:h-8 text-[8px] md:text-[10px] flex items-center justify-center font-bold transition-all duration-200 ";
    ...
    if (isToday) {
      classes +=
        "bg-black text-white shadow-lg cursor-pointer hover:scale-110 ";
      return classes;
    }
    ...
    if (assignTasks.length > 0) {
      classes +=
        "bg-blue-500 text-white shadow-lg cursor-pointer hover:scale-110 ";
      return classes;
    }
    ...
    if (deadlineTasks.length > 0) {
      classes +=
        "bg-red-500 text-white shadow-lg cursor-pointer hover:scale-110 ";
      return classes;
    }
...
<div className="flex flex-row flex-wrap items-center justify-between mb-4 md:mb-6 w-full">
  {/* Heading */}
  <h2 className="text-left font-bold text-gray-800 text-lg sm:text-xl md:text-3xl mb-3 md:mb-0 mr-4">
  ...
  <div className="flex items-center justify-end gap-2 md:gap-4 ml-auto">
    <div className="relative" ref={dropdownRef}>
      ...
      {showDropdown && (
        <div className="absolute top-full mt-1 md:mt-2 left-0 bg-white rounded-lg shadow-lg z-10 w-40">
          {[
            ...
            { label: "Month Calendar", href: "/monthcalendar" },
      ...
      {/* Calendar Grid */}
      <div className="grid grid-cols-2 px-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-4 text-xs mx-auto w-full">
        {months.map((month, monthIndex) => {
          ...
          return (
            ...
            >
              <h2 className="text-center font-semibold mb-4 md:mb-1 text-xs md:text-sm">
                {month.toUpperCase()}
              </h2>
              <div className="grid grid-cols-7 gap-0.5 md:gap-1 text-center font-medium text-gray-700 mb-1">
                {daysShort.map((day) => (
                  <div
                    key={day}
                    className="shadow-sm rounded py-0.5 md:py-1 text-[8px] md:text-[10px] font-bold"
                  >
                    {day}
                  </div>
                ))}
              ...
              <div className="grid grid-cols-7 gap-1 mb-2 md:mb-0 md:gap-1 text-center">
                {daysArray.map((day, i) => (
                  <div
                    ...
                    onClick={() =>
                      day && handleDateClick(baseYear, monthIndex, day)
                    }
                  >
                    {day}
                  </div>
              ...
              <div className="mb-4">
                ...
                <div className="space-y-2 md:space-y-3">
                  {selectedDateInfo.assignTasks.map((task) => (
                    <div
                      key={task._id}
                      className="bg-blue-50 p-2 md:p-4 rounded-lg border-l-2 md:border-l-4 border-blue-500"
                    >
                      <div className="font-semibold text-blue-800 text-sm md:text-lg">
                        {task.bucketName}
                      </div>
              ...
              <div>
                ...
                <div className="space-y-2 md:space-y-3">
                  {selectedDateInfo.deadlineTasks.map((task) => (
                    <div
                      key={task._id}
                      className="bg-red-50 p-2 md:p-4 rounded-lg border-l-2 md:border-l-4 border-red-500"
                    >
                      <div className="font-semibold text-red-800 text-sm md:text-lg">
                        {task.bucketName}
                      </div>
                      <div className="text-xs md:text-sm text-gray-600 mt-1 md:mt-2">
                        <div>

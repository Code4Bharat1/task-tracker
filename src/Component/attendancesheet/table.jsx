...
export default function AttendanceTable() {
  ...
  const fetchAttendanceData = async () => {
    ...
    try {
      ...
      if (res.data.role === 'TeamLeader') {
        response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_API}/attendance/teamAttendance`, {
          withCredentials: true, // Send cookies
        });
      } else if (res.data.role === 'Manager' || res.data.role === 'HR') {
        response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_API}/attendance/allAttendance`, {
          withCredentials: true,
        });
      } else {
        throw new Error("Unauthorized position to fetch attendance");
      }
      ...
      if (response && response.data && Array.isArray(response.data)) {
        const processedData = response.data.map((item) => ({
          ...
          workHours: item.totalWorkedHours || "N/A",  // Use totalWorkedHours instead of punchInLocation?
        }));
  ...
  const formatLocation = (fullAddress) => {
    ...
    const parts = (typeof fullAddress === 'string' ? fullAddress : "")
      .split(',')
      .map(part => part.trim())
